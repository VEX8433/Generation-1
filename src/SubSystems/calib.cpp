...
// External reference to chassis defined in main.cpp
...
#define CAL_GET_POSE_X() chassis.getPose().x
#endif
#ifndef CAL_GET_POSE_Y
#define CAL_GET_POSE_Y() chassis.getPose().y
#endif
#ifndef CAL_GET_POSE_H
#define CAL_GET_POSE_H() chassis.getPose().theta
#endif
#ifndef CAL_SET_POSE
...
void run_calibration_tests() {
  const char* profiles[] = {"normal", "fast"};
  const double caps[] = {0.75, 1.00};
  const double drive_in[] = {24.00, 48.00};
  const double turn_deg[] = {45.00, 90.00};
  const int repeats = 4;
